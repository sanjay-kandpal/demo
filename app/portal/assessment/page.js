...
const steps = [
  { id: "income", label: "Income Schedule", icon: "table_rows" },
  { id: "applicant", label: "Applicant & Loan Inputs", icon: "person" },
  { id: "results", label: "Assessment Results", icon: "grading" },
];

const aed = (value) => `AED ${Math.round(value).toLocaleString()}`;
...
function Stepper({ current, onSelect }) {
  return (
    <div className="bg-surface border border-surface-variant rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.05)] px-6 py-4">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <Fragment key={step.id}>
              ...
              >
                <span
                  className={`flex items-center justify-center w-9 h-9 rounded-full font-label-bold text-label-bold transition-colors duration-300 ${
                    isCurrent
                      ? "bg-primary text-on-primary"
                      : isDone
                        ? "bg-primary-fixed text-on-primary-fixed"
                        : "border-2 border-outline-variant text-on-surface-variant group-hover:border-secondary group-hover:text-secondary"
                  }`}
                >
                  {isDone ? (
                ...
                <span
                  className={`hidden md:inline font-label-bold text-label-bold transition-colors duration-300 ${
                    isCurrent ? "text-primary" : "text-on-surface-variant group-hover:text-secondary"
                  }`}
                >
                  {step.label}
                </span>
...
export default function BeneficiaryAssessmentPage() {
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-body-md text-on-surface-variant">
            Recognised-income basis. Work through the steps — results recompute automatically.
          </p>
        </div>
        <StatusPill
          ok={allVerified}
      ...
      {/* Keyed by step so each change re-runs the slide-in animation. */}
      <div key={step} className="step-enter space-y-8">
        {step === 0 && (
          <SectionCard
        ...
        {step === 2 && (
          <>
            ...
            <SectionCard title="Eligibility Gates" aside={<GatePill pass={result.gates.passed} />}>
        ...
        <div className="flex items-center justify-between">
          {step > 0 ? (
            <button
              className="flex items-center gap-2 border-2 border-secondary text-secondary hover:bg-secondary hover:text-on-secondary font-label-bold text-label-bold py-3 px-6 rounded transition-colors duration-300"
              type="button"
          ...
          {step < steps.length - 1 && (
            <button
              className="flex items-center gap-2 bg-primary hover:bg-primary-container text-on-primary font-label-bold text-label-bold py-3 px-6 rounded transition-all duration-300 group"
              type="button"
              onClick={() => goToStep(step + 1)}
            >
              {steps[step + 1].label}
