...
const lookupIncomeBand = (recognizedIncome) => {
...
const maxTenureForIncome = (recognizedIncome) => {
...
export function assess(input) {
  ...
  const allowedTenure = Math.max(
    1,
    Math.min(requestedTenureYears, maxTenureForIncome(recognizedIncome), Math.max(maxTenureByAge, 1))
  );
  ...
  const incomeBand = lookupIncomeBand(recognizedIncome);
